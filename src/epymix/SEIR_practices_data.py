...
def SEIR_practice(t, delta_t0, delta_t, season,
         disease, rain, inoc_init, ng_ext0, arrangement,
         lai_wheat_inc, lai_comp_inc, lai_wheat_mu,
         nu, ber_wheat, ber_companion, h_wheat, h_companion,
         lambd, delta_ei,
    ...
    Lr = arrangement.shape[0] # arrangement period
    ...
    for i in range(0, t):
        ...
        if np.mod(i, season) == 0:
            ...
            #res_frac = arrangement[int((np.floor(i / season) % Lr)), :, :]  # arrangement(:,:, year of arrangement)
            ...
            Pth[i, :, :] = lai_comp_inc[i] * np.ones((Lx, Ly))
            Poi[i, :, :] = lai_comp_inc[i] * np.ones((Lx, Ly))
            Sth[i, :, :] = lai_wheat_inc[i] * np.ones((Lx, Ly))
            Sus[i, :, :] = lai_wheat_inc[i] * np.ones((Lx, Ly))
        ...
        # Main function after initialisation
            ...
            Pth[i,:,:] = pth + lai_comp_inc[i]
            Poi[i,:,:] = poi + lai_comp_inc[i]
            Sth[i,:,:] = sth + lai_wheat_inc[i]
            Sus[i,:,:] = np.maximum(0, sus + lai_wheat_inc[i] * pow(((sus+lat)/(sth+(sth==0))),gamma) - Sc)

            Eta[1:lambd+delta_ei,:,:] = eta[0:-1,:,:] * (1 - g[0:-1,:,:])
            Eta[0,:,:] = Sc
            Eta = (1 - lai_wheat_mu[i] * (lai_wheat_mu[i] > 0)) * Eta
            Lat[i,:,:] = Eta.sum(axis=0)

            if disease == "septo":
                Ifc[i, :, :] = ifc * (1 - psi * pluie) - nu * ifc + np.sum((eta * g), axis=0)
                Rem[i,:,:] = rem + ifc * psi * pluie + np.sum((lai_wheat_mu[i] * (lai_wheat_mu[i] > 0) * Eta), axis=0) +\
                             lai_wheat_mu[i] * (lai_wheat_mu[i] > 0) * sus + nu * ifc
            elif disease == "rust":
                Ifc[i,:,:] = ifc - nu * ifc + np.sum((eta * g), axis=0)
                Rem[i,:,:] = rem + np.sum(lai_wheat_mu[i] * (lai_wheat_mu[i] > 0) * Eta, axis=0) +\
                             lai_wheat_mu[i] * (lai_wheat_mu[i] > 0) * sus + nu * ifc
            ...
            LAI_alive[i, :, :] = Sus[i, :, :] + Lat[i, :, :] + Ifc[i, :, :]
            LAI_wheat[i, :, :] = Sus[i, :, :] + Lat[i, :, :] + Ifc[i, :, :] + Rem[i, :, :]
            Poo[i, :, :] = poo - inoc + (1 - eps) * nsp + (1 - pi_inf0) * eps * nsp - rho * poo
            AUDPC[i,:,:] = audpc + Ifv[i,:,:]
            Nsp[i,:,:] = nsp
            # if ber_companion == 0:
